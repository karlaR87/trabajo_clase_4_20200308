...
export default function PokemonAxios() {
  ...
  const [nPokemon, setNPokemon] = useState(25); // Start with 25 Pokémon
      ...
      const pokemonDetails = await Promise.all(
        dataPokemon.map(async (pokemon) => {
          ...
          return {
            ...
            abilities: pokemonResponse.data.abilities.map((ability) => ability.ability.name).join(', '),
          };
        })
      );
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      <Text>Número Pokedex: <Text style={styles.number}>{item.id}</Text></Text>
      <Image
        style={styles.image}
        source={{ uri: `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${item.id}.png` }}
      />
      <Text style={styles.title}>{item.name}</Text>
      <Text style={styles.description}>Abilities: {item.abilities}</Text>
    </View>
  );
